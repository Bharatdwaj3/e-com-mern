...
import { Container, Box, Typography, Button, Skeleton, Divider } from "@mui/material";
...
const ProductDetails = () => {
  ...
  if (!product) return null; // redirect handled above
      ...
      <Container maxWidth="lg" className="pt-24 pb-12">
        ...
        <Box className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <Box className="grid md:grid-cols-2 gap-8 p-6">
            ...
            <Box>
              <img
                src={product.imageUrl?.trim() || "/image/product_not_available.jpg"}
                alt={product.type}
            ...
            <Box>
              ...
              <Box className="space-y-3 text-gray-700">
                ...
                <Typography><strong>Usage:</strong> {product.usage || "—"}</Typography>
                {product.size && <Typography><strong>Size:</strong> {product.size}</Typography>}
                {product.color && <Typography><strong>Color:</strong> {product.color}</Typography>}
                {product.material && <Typography><strong>Material:</strong> {product.material}</Typography>}
                {product.power !== undefined && (
                  <Typography><strong>Power:</strong> {product.power ? "Yes" : "No"}</Typography>
                )}
                {product.port && <Typography><strong>Port:</strong> {product.port}</Typography>}
                {product.wired !== undefined && (
                  <Typography><strong>Connection:</strong> {product.wired ? "Wired" : "Wireless"}</Typography>
                )}
                {product.display !== undefined && (
                  <Typography><strong>Display:</strong> {product.display ? "Yes" : "No"}</Typography>
                )}
                {product.storage != null && (
                  <Typography><strong>Storage:</strong> {product.storage} GB</Typography>
                )}
              </Box>
